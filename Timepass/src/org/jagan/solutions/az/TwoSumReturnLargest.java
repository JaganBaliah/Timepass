...
public class TwoSumReturnLargest {
	...
	public static int[] solution(int[] input, int target) {
		...
		for(int val : input) {
			...
			if((val > largest || complement > largest) && numIndexMap.containsKey(complement)) {
				largest = Math.max(val, complement);
				rtnVal[0] = i;
				rtnVal[1] = numIndexMap.get(complement);
			}
			numIndexMap.put(val, i);
			i++;
		}
		return rtnVal;
	}
